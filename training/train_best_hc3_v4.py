...
MODEL_PATH = "hc3_v4_best_model.joblib"
RESULTS_PATH = "hc3_v4_best_results.json"
...
def choose_threshold_max_recall_at_5pct_fpr(y_true, scores):
    ...
    for threshold in candidates:
        ...
        if fpr <= 0.05 + 1e-12:
            ...
            if best is None:
                best = item
            elif item["recall"] > best["recall"]:
                best = item
            elif (
                item["recall"] == best["recall"]
                and item["threshold"] > best["threshold"]
            ):
                best = item

    if best is None:
        # Conservative fallback: threshold above the largest negative score.
...
threshold_info = choose_threshold_max_recall_at_5pct_fpr(
    y_valid,
    valid_scores,
)
...
def metrics_for_scores(y_true, scores, threshold):
    pred = (scores >= threshold).astype(int)
...
train_metrics = metrics_for_scores(
    y_train,
    train_scores,
    threshold,
)

valid_metrics = metrics_for_scores(
    y_valid,
    valid_scores,
    threshold,
)
...
test_copy["pred"] = (test_scores >= threshold).astype(int)
...
artifact = {
    ...
    "model": "LinearSVC",
    "C": 10.0,
    "threshold_rule": "max recall subject to validation FPR <= 5%",
    "threshold": float(threshold),
